...
final as (
    select
        ...
        p."Title"                 as title,
        p."Body"                  as body,
        -- "|a|b|c|" -> ['a','b','c']. string_split(NULL,...) is NULL, so posts
        -- with no tags (answers) get a NULL array, not an empty one — the
        -- list_filter drops the empty leading/trailing elements the pipes create.
        list_filter(string_split(p."Tags", '|'), x -> x <> '') as tags_array,
